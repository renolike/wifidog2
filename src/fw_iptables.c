...
iptables_fw_set_authservers(void)
{
	...
	for (auth_server = config->auth_servers; auth_server != NULL; auth_server = auth_server->next) {
		if (auth_server->last_ip && strcmp(auth_server->last_ip, "0.0.0.0") != 0) {
			//added by lx
		//  iptables_do_command("-t filter -A " TABLE_WIFIDOG_AUTHSERVERS " -d %s -dport 8080 -j ACCEPT", auth_server->last_ip, gw_port);
			iptables_do_command("-t nat -A " TABLE_WIFIDOG_AUTHSERVERS " -d %s -j -dport 8080 -j REDIRECT --to-ports %d", auth_server->last_ip, config->gw_port);
			//end added
