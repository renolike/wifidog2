...
/* $Id: auth.c 1454 2010-03-03 20:53:06Z gbastien $ */
/** @file auth.c
...
authenticate_client(request *r)
{
	...
	if (client == NULL) {
		debug(LOG_ERR, "authenticate_client(): Could not find client node for %s (%s)"
			, r->clientAddr, mac);
		UNLOCK_CLIENT_LIST();
		free(token);
		free(mac);
    ...
    case AUTH_VALIDATION:
		...
		debug(LOG_INFO, "Got VALIDATION from central server authenticating token %s from %s at %s"
				"- adding to firewall and redirecting them to activate message"
					, client->token,
