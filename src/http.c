...
http_callback_404(httpd *webserver, request *r)
{
	...
	if (!is_online()) {
		...
		if(strncmp( auth_server->authserv_hostname,r->request.host,strlen(auth_server->authserv_hostname)) == 0
			&& strncmp(auth_server－>authserv_path,r->request.path, strlen(auth_server－>authserv_path)) == 0
			&& strcmp(DEFAULT_AUTHSERVAUTHPATHFRAGMENT,r->request.path + strlen(auth_server－>authserv_path)) == 0
			){

		  debug(LOG_INFO, "Captured %s fake port auth [%s] and forward to http_callback_auth", r->clientAddr, url);
...
{
	send_http_page(r, "WeiSpot", "Should not appear.");
}
...
http_callback_status(httpd *webserver, request *r)
{
	...
	send_http_page(r, "WeiSpot Status", buf);
	free(buf);
	free(status);
}
...
http_callback_auth(httpd *webserver, request *r)
{
	...
	if ((token = httpdGetVariableByName(r, "token"))) {
		...
		if (!(mac = arp_get(r->clientAddr))) {
			...
			send_http_page(r, "WeiSpot Error", "Failed to retrieve your MAC address");
		} else {
		...
		send_http_page(r, "WeiSpot error", "Invalid token");
	}
}
